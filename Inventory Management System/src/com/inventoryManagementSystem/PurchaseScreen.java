...
//		panel.setPreferredSize(new Dimension(1280,250));
		...
		JLabel lblPurchaseInvoice = new JLabel("PURCHASE INVOICE");
		...
		comboBoxVendor.setFont(new Font("Lato", Font.PLAIN, 16));
		panel.add(comboBoxVendor, "cell 1 3,grow");
		...
		textFieldID.setText(App.purchaseInvoices.size()+1+"");
		
		JLabel label = new JLabel("IN STOCK");
		label.setForeground(Color.WHITE);
		label.setFont(new Font("Montserrat", Font.BOLD, 16));
		panel.add(label, "flowx,cell 4 5");
	...
	public void actionPerformed(ActionEvent event) {
		...
		if(command.equals("ADD")) {			
			...
			if(comboBoxVendor.getSelectedIndex() != 0) {
			...
			if(comboBoxVendor.getSelectedIndex() != 0) {
				if(table.getRowCount() != 0) {
					...
					str.append("<----------------------------------------- PURCHASE INVOICE ----------------------------------------->\n");
					str.append(String.format("\nVendor: %s", comboBoxVendor.getSelectedItem()));
					...
					ArrayList<InvoiceItem> items = App.purchaseInvoices.get(id).getInvoiceData();
	...
	/**
	 * Function to save invoice in the ArrayList APP.purchaseInvoices
	 */
	private void saveInvoice() {
		...
		Vendor v = App.vendors.get(comboBoxVendor.getSelectedIndex()-1);
		
		App.purchaseInvoices.add(new Invoice(id, dateChooser.getDate(), v , invoiceItems));
	}
	...
	private void updateVendors() {
		...
		comboBoxVendor.setModel(model);
		if(App.vendors.size()<1)
			comboBoxVendor.setEnabled(false);
		else
			comboBoxVendor.setEnabled(true);
