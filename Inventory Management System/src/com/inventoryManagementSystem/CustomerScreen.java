...
public class CustomerScreen extends JFrame implements ActionListener{
	...
	public CustomerScreen() {
		...
		lblCustomers.setBounds(138, 58, 173, 31);
		lblCustomers.setAlignmentX(Component.CENTER_ALIGNMENT);
		lblCustomers.setForeground(Color.WHITE);
		lblCustomers.setFont(new Font("Montserrat", Font.BOLD, 30));
...
//					System.out.println("Exception was Cought!");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(command.equals("New")) {
			NewCustomerDialog screen = new NewCustomerDialog();
			...
			try {
				...
				int index = searchCustomerById(id);
				...
				if(index != -1) { 
					App.customers.get(index).setName(name);
					App.customers.get(index).setPhoneNumber(phone);
					App.customers.get(index).setAddress(address);
					refreshTable(App.customers);
					emptyFields();
				}
			...
			if(searchStr.length() != 0) {
				...
				for(Customer c : App.customers) {
					if(c.getName().toUpperCase().contains(searchStr.toUpperCase())) {
						customers.add(c);
					}
				}
	...
	public void refreshTable(ArrayList<Customer> data) {
		...
		for(Customer customer : data) {
			model.addRow(new Object[] {customer.getID(),customer.getName(),customer.getPhoneNumber(),customer.getAddress()});
		}
	...
	private int searchCustomerById(int id) {
		
		for(Customer customer : App.customers) {
			if(customer.getID() == id) {
				return App.customers.indexOf(customer);
			}
		}
		return -1;
	}
	private void fillData(int id) {
		int index = searchCustomerById(id);
		try {
		textFieldId.setText(App.customers.get(index).getID()+"");
		textFieldName.setText(App.customers.get(index).getName());
		textFieldPhone.setText(App.customers.get(index).getPhoneNumber());
		textAreaAddress.setText(App.customers.get(index).getAddress());
		}catch(ArrayIndexOutOfBoundsException ex) {
